...
from frontend.util import read_app_log


class WINDOW_read_app_log(CTkToplevel):
    def __init__(
        self,
    ...
    def _fill_log(self) -> None:
        try:
            self._log_text.insert(END, read_app_log())
            self._log_text.configure(state="disabled")
        except Exception as e:
            logging.error(f"Could not read application log file. Detail: {e}.")
            messagebox.showerror(
                "Erro ao ler log",
                f"Ocorreu um erro ao ler o log da aplicação. Detalhe: {e}.",
            )
...
def create_window(master) -> None:
    if not path.exists(
        ...
        messagebox.showwarning(
            ...
            f"Nenhum log encontrado para a aplicação.",
        )
        return

    WINDOW_read_app_log(master)
